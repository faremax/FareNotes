...
- [闭包](#%E9%97%AD%E5%8C%85)
	...
	- [闭包的优缺点：](#%E9%97%AD%E5%8C%85%E7%9A%84%E4%BC%98%E7%BC%BA%E7%82%B9%EF%BC%9A)
...
```
![这里写图片描述](http://img.blog.csdn.net/20161117161223951)
...
1.函数嵌套函数
2.函数内部可以引用外部的参数和变量
3.参数和变量不会被垃圾回收机制回收(内存管理和垃圾回收以后会专门讲一篇的)<!--TODO-->
...
2. 使用不当很容易造成内存泄露(关于内存管理和垃圾回收的细节以后会专门讲一篇的)。<!--TODO-->
...
箭头函数本身也是一个函数，具有自己的作用域。不过在箭头函数里面的this上下文同函数定义所在的上下文，具体可以看我的另一篇文章：[javascript中this详解](http://blog.csdn.net/faremax/article/details/53235837)

###典型实例
这个实例会涉及到对象的相关知识，如果不能完全理解，可以参考：[javascript中this详解](http://blog.csdn.net/faremax/article/details/53235837) 和 [javascript对象、类与原型链](http://blog.csdn.net/faremax/article/details/53525721)
```
function Foo() {
